      ...
      <div className="glass-panel p-5 relative overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white font-heading flex flex-wrap items-center gap-2">
                <span>{t('catalogue.title', 'Artisan Product Catalogue')}</span>
                <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                  {products.length} {t('catalogue.colProduct', 'Items')}
                </span>
                {!isOnline && (
                  <span className="text-[10px] font-mono px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 flex items-center gap-1 font-semibold animate-pulse">
                    <WifiOff className="w-3 h-3" /> Offline (Edge SQLite Mode)
                  </span>
                )}
              </h2>
                  ...
                  <div className="absolute top-2.5 left-2.5 flex flex-wrap gap-1.5">
                    ...
                    {product.offlineQueued || product.syncedWithDb === false ? (
                      <span className="bg-amber-950/90 backdrop-blur-sm text-amber-300 text-[9px] font-semibold px-2 py-0.5 rounded-full border border-amber-500/40 flex items-center gap-1 font-mono">
                        <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span> Queued in SQLite
                      </span>
                    ) : (
                      <span className="bg-emerald-950/80 backdrop-blur-sm text-emerald-300 text-[9px] font-semibold px-2 py-0.5 rounded-full border border-emerald-500/30 flex items-center gap-1 font-mono">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span> Main DB Synced
                      </span>
                    )}
                  </div>
